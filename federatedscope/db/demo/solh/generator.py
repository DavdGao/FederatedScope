...
import os
print(f"Write data into {os.path.abspath('data')}")

empty_df = pd.DataFrame(columns=['value'])
empty_df.to_csv('data/server.csv', index=False, float_format="%.2f")
empty_df.to_csv('data/shuffler.csv', index=False, float_format="%.2f")
...
for part in part_size:
  top += part
  cid += 1
  df.iloc[base:top].to_csv('data/client%d.csv' % cid, index=False, float_format="%.2f")