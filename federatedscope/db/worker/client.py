...
class Client(Worker):
    ...
    def upload_data(self):
        ...
        """
        logger.info(
            f"Send encrypted accessor to the server with epsilon={self._cfg.ldp.epsilon}, fanout={self._cfg.ldp.fanout}."
        )
        (_, encoded_table) = self.sql_processor.encode_table(
            self.sql_accessor.get_table(), self._cfg.ldp.epsilon,
            self._cfg.ldp.fanout)
